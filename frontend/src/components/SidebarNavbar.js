...
const SidebarNavbar = () => {
    ...
    const [noUserSidebar, setNoUserSidebar] = useState(false)
    const [listNotification, setListNotification] = useState([])
    const webSocket = new WebSocket(
        `ws://${process.env.REACT_APP_BACKEND_URL}/ws/notifications/`
    )
    ...
    const handleChangeNoUserSidebar = (e)=>{
        setNoUserSidebar(!noUserSidebar)
        console.log('no user sidebar')

        console.log(noUserSidebar)
    }
    ...
    return (
    <> 
        <nav class={user ? "fixed h-[80px] top-0 z-50 w-full md:pt-[15px] bg-white border-b border-gray-200 text-gray-800": "fixed h-[80px] top-0 z-50 w-full bg-white border-b border-gray-200 text-gray-800"}>
        <div class="">
            <div class={user ? "flex items-center justify-between px-3 pt-[18px] md:pt-[5px]" : "flex items-center justify-between px-3 pt-[18px] md:pt-[5px]"}>
                <div class="flex items-center justify-start">
                    <button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" class="cursor-pointer inline-flex items-center p-2 text-sm text-gray-500 rounded-lg md:hidden focus:outline-none focus:ring-2 focus:ring-gray-200  ">
                        <span class="sr-only">Open sidebar</span>
                        <svg class="w-6 h-6" onClick={user ? handleChangeSidebar : handleChangeNoUserSidebar} aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="self-center cursor-pointer text-xl no-underline font-semibold sm:text-2xl whitespace-nowrap text-[#ff792e]">
                        <Link to={user? '/home' : '/'} className='no-underline text-[#ff792e] hover:text-[#ff792e]'>  
                ...
                {!user && 
                    ...
                    <ul className='hidden md:flex pt-2 p-0'>
                        <li className='hover:text-[#ff792e] font-bold cursor-pointer'>Home</li>
                        <li className='hover:text-[#ff792e] font-bold cursor-pointer'>About</li>
                        <li className='hover:text-[#ff792e] font-bold cursor-pointer'>Features</li>
                        <li className='hover:text-[#ff792e] font-bold cursor-pointer'>Pricing</li>
                    </ul>
                    </div>
                }
                        ...
                        <div class="flex items-center ml-3 relative">
                            <div>
                                <button onClick={handleUserState} type="button" class="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300 " aria-expanded="false" data-dropdown-toggle="dropdown-user">
                                    ...
                                    <img class="w-8 h-8 rounded-full object-cover" src={currentUser && currentUser.image} alt="user photo" />
                                </button>
                            </div>
                            <div class={userState ? "z-50 absolute top-4 right-0 w-[300px] my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow " : 'hidden'} id="dropdown-user">
                                <div class="px-4 py-3 flex items-center" role="none">
                                    <img class="w-10 h-10 rounded-full object-cover" src={currentUser && currentUser.image} alt="user photo" />
                    ...
                    (
                        <div className='flex items-center '>
                            <button className='mr-2'>
                                <Link className="no-underline hover:text-[#ff792e] hidden md:block font-bold cursor-pointer text-[#67748E]" to='/login'>
                                    Sign in
                                </Link>
                            </button>
                            <button className='bg-[#ff792e]  hidden  md:block px-3 py-2 md:px-5 md:py-2 rounded-lg hover:opacity-80'>
        ...
        : !user ? (
        <aside id="logo-sidebar" class={noUserSidebar === true ? "fixed md:hidden top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 ": "fixed md:hidden top-0 left-0 z-40 w-64 h-screen pt-20 bg-white border-r border-gray-200"} aria-label="Sidebar">
        <div class="h-full overflow-y-auto bg-white ">
            <ul class="font-medium p-0">
                ...
                <li className='border-solid p-0'>
                    <a href="/login" class="flex items-center p-2 text-[#ff792e] rounded-lg no-underline hover:text-white  hover:bg-[#1b2850] ">
                    <span class="">Sign In</span>
                    </a>
                </li>                
                <li className='border-solid p-0'>
                    <a href="/register" class="flex items-center p-2 text-[#ff792e] rounded-lg no-underline hover:text-white  hover:bg-[#1b2850] ">
