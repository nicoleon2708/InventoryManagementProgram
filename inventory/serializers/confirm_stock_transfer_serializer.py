    ...
    def confirm_stock(self):
        ...
        if list_transfer_detail:
            for transfer_detail in list_transfer_detail:
                stock_transfer = TransferService.get_location_stock_of_product_at_location(
                    location=transfer.source_location, 
                    product=transfer_detail.product
                )

                if stock_transfer is None or stock_transfer.quantity == 0 or stock_transfer.quantity < transfer_detail.quantity:
                    continue
