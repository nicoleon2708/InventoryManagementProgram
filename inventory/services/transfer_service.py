...
class TransferService:  
    ...
    @staticmethod
    def create_transfer(user, outcome, source_location, destination_location, *args, **kwargs):
        transfer = Transfer(
            user=user,
            outcome=outcome,
    ...
    def check_match_transfer(transfer_list, match_rule):
        for transfer in transfer_list:
            if transfer.source_location == match_rule.source_location and transfer.destination_location == match_rule.destination_location:
                return transfer
        return None
    ...
    @staticmethod
    def create_transfer_and_transfer_detail_while_valid(match_rule, product, quantity, transfer_list, outcome, status, note=''):
        ...
        while match_rule:
            ...
            if match_rule.types_of_rule == 'ANT_LC':
                if stock_source_location is None or stock_source_location.quantity == 0 or stock_source_location.quantity < quantity:
                    ...
                    if stock_source_location and stock_source_location.quantity < quantity:
