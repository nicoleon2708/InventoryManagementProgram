...
class TestTransferViews(APITestCase, BaseModelTestCase):
    ...
    def test_get_detail_transfer(self):
        detail_url = reverse(
            "inventory:transfer-detail", kwargs={"pk": self.transfer.pk}
        )
    ...
    def create_purchase_invoice(self):
        purchase_detail = {"product": self.product.pk, "quantity": 300, "price": 20}
        data = {
            "user": self.user.pk,
        ...
        create_purchase_url = reverse("inventory:transfer-import_product")
        response = self.client_api.post(create_purchase_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_confirm_transfer(self):
        confirm_transfer = reverse(
            "inventory:transfer-confirm-stock-transfer", kwargs={"pk": self.transfer.pk}
        )
        response = self.client_api.post(confirm_transfer, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_confirm_purchase(self):
        confirm_purchase = reverse(
            "inventory:transfer-confirm-purchase", kwargs={"pk": self.transfer.pk}
        )
